...
<?php
	...
	$number = (int) $_GET['n'];
	//получаем вопрос
	$query_q = mysqli_query($con,"SELECT * FROM questions_css WHERE question_number = $number");

	$question = mysqli_fetch_assoc($query_q); //Извлекает результирующий ряд в виде ассоциативного массива

	//получаем варианты ответов
	$query_a = mysqli_query($con,"SELECT * FROM choices_css WHERE question_number = $number");
